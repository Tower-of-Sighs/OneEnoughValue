package cc.sighs.oneenoughvalue.server.recipe;
...
import cc.sighs.oneenoughvalue.server.recipe.api.RecipeHandler;
...
public class RecipeHandlerManager {
    ...
    public <C extends RecipeInput, T extends Recipe<C>> void parseAllRecipes(RecipeManager manager, RegistryAccess registryAccess) {
        currentRegistryAccess = registryAccess;
        init();
        for (Map.Entry<RecipeType<?>, RecipeHandler> entry : handlers.entrySet()) {
            for (RecipeHolder<T> recipe : manager.getAllRecipesFor((RecipeType<T>) entry.getKey())) {
                var node = new RecipeNode(entry.getValue(), recipe.value());
                allNodes.add(node);
                nodes.offer(node);
            }
        }
